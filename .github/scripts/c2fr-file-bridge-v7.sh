...
mapfile -t INDICES < <(seq -w 0 15)
READY=0
for _ in $(seq 1 120); do
  OK=1
  for index in "${INDICES[@]}"; do
    fetch_text_file "$PREFIX/payload-$index.b64" \
      "$RUNNER_TEMP/c2fr-bridge/payload-$index.b64" || OK=0
  done
  fetch_text_file "$PREFIX/key.b64" \
    "$RUNNER_TEMP/c2fr-bridge/key.b64" || OK=0
  if [[ "$OK" -eq 1 ]]; then
    READY=1
    break
  fi
  rm -f "$RUNNER_TEMP/c2fr-bridge"/payload-*.b64 \
    "$RUNNER_TEMP/c2fr-bridge/key.b64"
  sleep 5
done
[[ "$READY" -eq 1 ]]

cat "$RUNNER_TEMP/c2fr-bridge"/payload-*.b64 \
  | tr -d '\n' | base64 -d > "$RUNNER_TEMP/c2fr-bridge/payload.enc"
tr -d '\n' < "$RUNNER_TEMP/c2fr-bridge/key.b64" \
  | base64 -d > "$RUNNER_TEMP/c2fr-bridge/key.enc"
printf 'encrypted_payload_received=PASS\ntransport=sixteen-file-payload\npayload_prefix=%s\n' \
  "$PREFIX" >> "$EVIDENCE_DIR/key-exchange.txt"
